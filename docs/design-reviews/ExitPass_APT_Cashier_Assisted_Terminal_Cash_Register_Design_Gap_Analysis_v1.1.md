...
Target branch: `docs/correct-optional-cash-drawer-posture`

Scope: Focused design correction only. This update preserves the v1.1 Cashier-Assisted Terminal gap analysis while correcting cash-drawer assumptions. It does not perform another repository audit, reopen the completed gap analysis, define production code, create database migrations, change the terminal shell, analyze Continuity Terminal behavior, push, or create a pull request.
...
The Cashier-Assisted Terminal is the Windows fixed-station Cashier POS Client and computerized Cash Register Terminal for normal staffed parking operations. It is the focus of this analysis. Current parking-site cashiers use ordinary PCs without electronic cash drawers, so cash tendering must work without cash-drawer hardware. The current Assisted Payment Terminal BRD/system design and current APT repository correctly preserve the major ExitPass authority boundaries, but they stop at generic "payment-capable terminal" behavior and the existing implementation stops before payment collection. They do not define the cash-register responsibilities now required for physical cash tender: local durable custody evidence, cash-custody session accounting, optional cash-drawer evidence, cash tender state, cashier attestation, restart recovery, cash-specific idempotency, receipt print journaling, and reconciliation ownership.
...
The Cashier-Assisted Terminal remains separate from Operator Console. Cash-drawer integration is optional, configurable, and disabled by default through a business/configuration posture equivalent to `CASH_DRAWER_ENABLED=false`. A cash drawer is not a prerequisite for starting a cash-custody session, reaching `CASH_RECEIVED`, or accepting cash. All previously approved cash, shift, refund, fiscal, receipt, authority, and Operator Console decisions remain unchanged by this correction except that drawer support is now explicitly optional and supplemental.
...
| Source | Purpose | Version/status | Authority posture | Cash tender relevance | Local DB relevance | Shift/custody relevance | Central PMS finality relevance | POS Server fiscal relevance | Contradictions or missing detail |
| --- | --- | --- | --- | --- | --- | --- | --- | --- | --- |
...
| Physical cash received | Cashier attestation at terminal | APT local DB | APT local DB for physical fact | Central PMS cash payment record references | APT with cashier identity | Must reconcile to Central PMS confirmation and cash-custody session |
| Amount tendered | Cashier entry/denomination count | APT local DB | APT local DB for tender fact; Central PMS for canonical payment amount after accepted | Central PMS cash command | APT before final local receive; correction requires reversal/refund event | Cash-custody and payment reconciliation |
| Denomination breakdown | Cashier/optional device entry | APT local DB | APT local DB | Central PMS/OC optional summary | APT | Cash-custody count variance |
| Change due | APT local calculation from payable and amount tendered | APT local DB | APT local DB for local tender computation | UI/backend summary | APT deterministic calculation | Cash-custody balance |
| Change given | Cashier attestation | APT local DB | APT local DB | Central PMS/OC summary | APT | Cash-custody balance/dispute evidence |
| Cash-custody session | Cashier opens local custody context | APT local DB | APT local DB for terminal-local custody facts; Central PMS/OC for supervisory state | OC summaries | APT local with supervisor controls | Shift/cash-custody reconciliation |
| Optional cash-drawer configuration | Site/terminal configuration | Central PMS/approved config plus APT local cache | Approved configuration source | APT cache | Approved operations/admin workflow | Confirm `CASH_DRAWER_ENABLED=false` default or enabled device binding |
| Enabled cash-drawer device | Approved terminal/device binding | Approved configuration source plus APT local device evidence | Approved configuration source for binding; APT for local device events | OC health/event view | Approved operations/admin workflow | Device binding and local event reconciliation |
| Drawer-open request/result/fault/no-sale event | Optional enabled cash-drawer device | APT local DB | APT local DB for supplemental device fact | OC health/event view | APT/device integration records result | Supplemental evidence; drawer-open success does not prove cash receipt |
...
| No-sale drawer open | Cashier/supervisor action/device result when cash-drawer capability is enabled | APT local DB | APT local DB | OC alert/summary | APT/device integration | Fraud monitoring |
| Paid-out | Cashier/supervisor action | APT local DB, Central PMS/OC approval if policy | APT local DB for physical payout | OC/reconciliation | APT with approval | Cash-custody and finance review |
| Safe drop/cash turnover | Cashier/supervisor action | APT local DB, Central PMS/OC visibility | APT local DB for physical removal | OC/reconciliation | APT with approval | Cash-custody closing |
...
| --- | --- | --- |
| `TENDER_STARTED` | Cashier selected cash for a current payable basis under an active cash-custody session | `TENDER_CANCELLED`, `CASH_RECEIVED` |
...
| `SUPERVISOR_REVIEW_REQUIRED` | Local custody, correction, refund, or optional device ambiguity requires review | `TENDER_CANCELLED`, `CASH_RECEIVED`, `RECONCILIATION_REQUIRED` |
...
Conditional drawer event subflow:

- Cash-drawer integration is configured off by default through a posture equivalent to `CASH_DRAWER_ENABLED=false`.
- When cash-drawer support is disabled, cash tendering remains available, no drawer-open command is required, no drawer-open device evidence is expected, and `CASH_RECEIVED` depends only on cashier attestation plus durable local commit.
- When cash-drawer support is enabled, the configured drawer device may be associated with the cash-custody session and APT may record drawer-open request, result, timeout, fault, and no-sale-open events as supplemental terminal-local evidence.
- Drawer-open success does not mean cash was received. `CASH_RECEIVED` remains the irreversible physical cash-custody point.
- Enabled drawer integration may require the local .NET device service when browser or WPF direct integration is not sufficient or approved.
...
Cash is physically received only when the cashier attests receipt and the terminal commits a local `CASH_RECEIVED` event with amount, payable basis reference, cashier, authenticated cashier session, work shift, cash-custody session, terminal, site, site group, POS Server, timestamp, correlation id, and any denomination/change facts. Pressing a generic "cash" button, opening a physical drawer, or receiving drawer-open success is not sufficient.
...
- Before `CASH_RECEIVED`: cashier may cancel without reversal, but any optional cash-drawer events remain in the local journal.
...
| --- | --- |
| Before cash received | Fail closed. No payment submitted. Keep local tender and optional device events. |
| Optional drawer event ambiguous | Cash tendering can continue only through explicit cashier/supervisor decision; drawer evidence remains supplemental and does not decide cash receipt. |
...
Availability checks before cash tender:

- Current payable basis not expired.
- Cashier authenticated and cash-custody session open.
...
- POS Server/fiscal readiness check should pass before cash tender for normal Cashier-Assisted Terminal operation. Recommendation: do not accept cash if POS Server is unavailable or fiscal path is known blocked, unless an approved supervisor exception exists.
- Cash drawer availability is not an availability check unless optional cash-drawer capability is explicitly enabled for that terminal.
...
| `cash_custody_sessions` | Primary cash accounting context: opening cash balance, opening denomination count, cash tenders, change given, refunds, paid-outs, safe drops/cash turnovers, adjustments, closing cash count, and overage/shortage | Local authoritative for terminal cash-custody facts | Financial retention | Encrypt | Summary | cashier id, auth session id, shift id, terminal id, site id, site group id, POS Server id |
| `cash_drawer_capability` | Optional cash-drawer capability and configured enabled device binding | Config cache plus local device evidence; disabled by default | While configured plus audit trail | Encrypt if device metadata is sensitive | Yes | config id, drawer device id |
...
| `cash_denomination_entries` | Denomination counts | Local authoritative | Financial retention | Encrypt | Summary | tender/custody count id |
...
| `device_events` | Optional cash drawer, printer, scanner, and other device events | Local authoritative | Operational/financial retention | Encrypt if tied to transaction | Summary | device id, event id |
...
2. Persist any optional enabled cash-drawer request/result/fault event if the terminal is configured for drawer integration.
3. Persist `CASH_RECEIVED` with cashier attestation, amount tendered, change due/given, cash-custody session, and immutable event id.
...
| Rejected | Enter `REFUND_REQUIRED` or reconciliation, depending on whether cash is still in local custody/customer present |

Current gap: Central PMS exposes a provider-oriented `POST /v1/public/payment-attempts` requiring `PaymentProvider` and returning provider handoff. It does not define a terminal cash-payment command with tender id, physical cash receipt facts, cash-custody session binding, amount tendered, change, cashier attestation, optional drawer event references, or local journal reference.

## 10. Cashier shift and cash-custody session model

Cashier shift and cash-custody session are separate objects.

Reason: a cashier shift is an identity/authorization/work-period concept. A cash-custody session is the primary terminal-local accounting container for opening cash balance, opening denomination count, cash tenders, change given, refunds, paid-outs, safe drops or cash turnovers, adjustments, closing cash count, and overage or shortage. A physical drawer device is optional. When cash-drawer support is enabled, the configured drawer hardware may be associated with the cash-custody session and contribute supplemental device evidence.

Recommended rules:

- One transaction must bind cashier, authenticated cashier session, work shift, terminal, cash-custody session, site, site group, and POS Server.
- One active cash-custody session per cashier terminal is the initial MVP posture.
- Current sites do not assume a physical drawer. Cash tendering must remain available when cash-drawer integration is disabled.
- If enabled, one physical drawer shared across cashiers should be disallowed in MVP unless supervisor handover is implemented.
...
| Cash-custody session open/close/count facts | APT local DB, synchronized summaries/exceptions to Central PMS/Operator Console |
| Optional enabled cash-drawer device facts | APT local DB, synchronized summaries/exceptions to Central PMS/Operator Console when configured |
...
- Cash tender totals to cashier shift and cash-custody session expected cash.
...
| Cash overage | `cash_custody_overage` | Site supervisor/finance |
| Cash shortage | `cash_custody_shortage` | Site supervisor/finance |
...
| --- | --- | --- | --- | --- | --- | --- |
| Cancellation before cash receipt | Cashier | No, unless optional device/custody ambiguity exists | APT local | Append `TENDER_CANCELLED` | None | Execute locally |
...
| Cash-custody adjustment | Cashier/supervisor | Required | APT local cash-custody facts | Append adjustment | Summary/reconciliation | Execute locally with approval |
...
The local .NET device service is optional until a confirmed peripheral requires it. It is not mandatory merely because cash tendering exists. Ordinary cashier PCs without electronic cash drawers can accept cash through cashier attestation and durable local cash-custody records.

Device-service posture:

- Cash-drawer integration is disabled by default through a posture equivalent to `CASH_DRAWER_ENABLED=false`.
- If cash-drawer support is enabled and browser/WPF direct integration is not sufficient or approved, the enabled cash-drawer device requires the local .NET device service.
- The same local service may also be required later for receipt printers, serial scanners, customer displays, payment devices, or secure device evidence.
- Simulators and hardware-in-loop validation are required only for the enabled peripheral capabilities being delivered.

| Device | Likely protocol | Preferred integration | Required evidence | Failure handling | Simulator | Stage |
| --- | --- | --- | --- | --- | --- | --- |
| Optional cash drawer | Printer kick pulse, OPOS/ESC-POS, serial/USB | Disabled by default; .NET local device service when enabled and direct integration is not approved | command, result, timeout, drawer id, no-sale-open event | Treat as supplemental device evidence; do not block `CASH_RECEIVED` solely because drawer hardware is absent | Required when enabled | Future/enabled-terminal scope |
| Receipt printer | ESC/POS, Windows print queue, OPOS | Windows print path or .NET local device service depending on approved printer integration | job id, payload hash/ref, result, reprint flag | Print failed state; allow governed reprint | Required when printer integration is delivered | MVP/UAT |
...
| Barcode/QR scanner | HID keyboard, serial, camera | UI for HID; device service for serial/camera | scan source and value classification | Manual fallback | Required when scanner integration is delivered | MVP |
...
- Local device service: optional cash drawer when enabled, printer/scanner/device IO when the selected peripheral requires it, and secure local evidence where approved.
...
- Cash tender works without a cash drawer. Cash-drawer integration remains disabled by default unless explicitly configured for the terminal.
...
- Optional cash-drawer open rate alerts only for terminals with cash-drawer integration enabled.
...
- Cashier shift and cash-custody session visibility.
- Optional cash-drawer device health/event visibility only when cash-drawer integration is enabled.
...
| ExitPass/Central PMS | Terminal/shift/cash-custody validation API | Cashier POS control before cash tender | Design closure | controlled UAT |
...
| ExitPass/Operator Console | Cash-custody/shift/reconciliation supervisory views, plus optional cash-drawer health when enabled | Governance without payment collection | Controlled UAT | controlled UAT |
...
| APT-CR-GAP-006 | No cash tender state machine | APT System Design | Workflow/state | Generic payment/fiscal status | Cannot handle cash-custody/backend failures or optional device ambiguity | Duplicate tenders and ambiguous recovery | Add core state model in this report, with optional conditional drawer event subflow | Yes | APT | CRITICAL | blocks_cash_implementation | design closure |
| APT-CR-GAP-007 | Shift and cash-custody session conflated/undefined | APT BRD/System Design, OC docs | Shift/custody | Mentions shift/session but not cash-custody accounting | Cash custody cannot be closed/reconciled | Overage/shortage untraceable | Separate cashier shift and cash-custody session model; associate drawer device only when enabled | Yes | APT + ExitPass | HIGH | blocks_first_cash_transaction | design closure |
...
| APT-CR-GAP-009 | Optional cash-drawer capability not configurable or journaled | APT repo | Device integration | No cash-drawer configuration or enabled-device journal | Future enabled drawers would lack device identification, command/result evidence, failure handling, simulator support, and hardware-in-loop validation | Device disputes where drawer integration is enabled | Add default-off cash-drawer configuration and optional `device_events` journal; require local device service only when enabled integration needs it | Yes | APT | HIGH | blocks_first_cash_transaction | cash implementation |
...
| APT-CR-GAP-011 | No rule for cash when Central PMS unavailable | APT/Continuity docs | Offline payment | Offline payment open | Cashier may receive unconfirmable cash | Uncontrolled receivables | Default fail closed for the Cashier-Assisted Terminal; explicit exception only | Yes | APT + ExitPass | HIGH | blocks_first_cash_transaction | design closure |
| APT-CR-GAP-012 | No rule for cash when POS Server unavailable | APT/POS/Continuity docs | Fiscal exception | Fiscal exceptions discussed generally | Paid customer may lack fiscal path | Exit block/customer dispute | Default fail closed before cash tender | Yes | APT + ExitPass | HIGH | blocks_first_cash_transaction | design closure |
| APT-CR-GAP-013 | Existing payment API cannot represent amount tendered/change | Central PMS `CreatePaymentAttemptRequest` | Public payment attempts | parking session, tariff snapshot, provider only | Cash overpay/change facts lost | Cash-custody mismatch | Add cash tender fields in separate endpoint | Yes | ExitPass | HIGH | blocks_cash_implementation | design closure |
...
| APT-CR-GAP-019 | Operator Console lacks cash-register governance views | OC BRD/System Design | APT relationship | Supervises generally, non-payment | No cash-custody/shift/cash exceptions | Blind supervision | Add read-only/supervisory cash views, plus optional cash-drawer health only when enabled | Yes | ExitPass | HIGH | blocks_controlled_uat | controlled UAT |
...
| APT-CR-GAP-022 | No paid-out, safe-drop/cash-turnover, adjustment, or optional no-sale model | APT docs | Cash-custody control | Omitted | Cash-custody operations unaccounted; optional no-sale drawer opens untracked when drawer is enabled | Shortage/overage ambiguity | Add cash-custody operation event types and approvals; include no-sale drawer events only for enabled drawers | Yes | APT | HIGH | blocks_controlled_uat | design closure |
...
| APT System Design | ExitPass | Workflow/state, local DB, optional device service, security, recovery | Define cash lifecycle and local durability, with cash-drawer integration default off | P0 | D-002 to D-007 |
...
| Central PMS API contracts | ExitPass | Cash payment command/readback, shift/cash-custody validation | Enable canonical cash payment safely | P0 | D-004 to D-006 |
...
| APT shift/reconciliation spec | APT | New design doc | Shift/cash-custody/reconciliation behavior, with optional drawer evidence | P1 | D-010, D-014 |
...
5. Shift/cash-custody session model, with optional cash-drawer capability disabled by default.
...
2. Cashier authentication, shift context, and cash-custody session skeleton.
...
10. Hardware-in-loop validation only for enabled peripheral integrations such as cash drawer, printer, or scanner.

Smallest safe first implementation slice after design revision:

> Implement the encrypted/local durable operational database foundation plus cash-custody session journal and cash tender state machine through `CASH_RECEIVED` in a non-live mode that cannot submit payment or open real hardware. Keep cash-drawer integration disabled by default; add simulator/device journals only when an optional enabled cash-drawer capability is included. This proves local durability, restart recovery, append-only events, and shift/cash-custody bindings before any real cash is accepted.
...
| D-005 | Accept cash when POS Server unavailable? | Yes fiscal-pending, no, supervisor exception | No for normal cash tender before physical cash custody | Avoids paid/no fiscal flow | Design closure |
| D-006 | Exact physical custody point? | Drawer opened, cashier confirms amount, backend confirms | Cashier confirms and local `CASH_RECEIVED` commits; drawer-open success is only supplemental evidence when enabled | Defines irreversible event | Design closure |
...
| D-010 | Shift and cash-custody relationship? | Same object, separate one-to-one, separate many-to-many | Separate; one active cash-custody session per cashier terminal in MVP; physical drawer optional | Supports future controls | Design closure |
| D-011 | Safe drops, cash turnovers, paid-outs, no-sale opens? | Exclude MVP, include with supervisor, include cashier-only | Include cash-custody model now; no-sale opens apply only when cash-drawer capability is enabled | Prevents cash-custody gaps | Design closure |
...
| D-014 | Windows device-service boundary? | WPF direct, separate local service, browser APIs | Local .NET device service is optional until a confirmed enabled peripheral requires it; required for enabled cash drawer only if direct integration is not approved | Testable and auditable without making cash drawer mandatory | Design closure |
